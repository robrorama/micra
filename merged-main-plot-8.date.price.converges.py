from data_retrieval import (
    get_stock_data, 
    add_moving_averages, 
    create_output_directory,  # <-- imported from data_retrieval
    get_ratio_dataframe       # <-- imported from data_retrieval
)
...
def main():
    ...
    output_directory = create_output_directory(tickers[0])  # Now called from data_retrieval.py
    ...
    slope_high, intercept_high = plot_projection_line(
        df, fig, high_peaks['High'], color='green', line_name='High Peak Line'
    )
    slope_low, intercept_low = plot_projection_line(
        df, fig, low_troughs['Low'], color='red', line_name='Low Trough Line'
    )
    ...
    try:
      ...
      intersection_price = y_intersect  # Capture the raw intersection price
    except ValueError as e:
      print("Skipping intersection marker:", e)

    title_suffix = f" | High Angle: {high_angle}° | Low Angle: {low_angle}°"

    # Only if we actually got an intersection:
    if intersection_date and intersection_price is not None:
      # Round or format as needed
      title_suffix += f" | Intersection: {intersection_date} @ {intersection_price:.2f}"
    ...
    intersection_date = None
    try:
        date_intersect, y_intersect = calculate_intersection(
